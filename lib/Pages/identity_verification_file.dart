...
const TextStyle _DESC_STYLE =
    TextStyle(fontFamily: 'Doomsday', fontSize: 13, color: MyColors.grey_color);
const TextStyle _DESC_BOLD_STYLE = TextStyle(
    fontFamily: 'Doomsday',
    fontSize: 13,
    color: Colors.black,
    fontWeight: FontWeight.bold);
...
class IdentityVerificationFileState extends State<IdentityVerificationFile>  with TickerProviderStateMixin, ImagePickerListener{
  ...
  void initState() {
    ...
    imagePicker!.init();
    verification_code = CommonUtils.getIDVerificationCode();
    if(verification_code == null || verification_code == ''){
      verification_code = CommonUtils.generateIDVerificationCode();
    }
    getVerification();
  }
  ...
  selectFile(int type) async {
    ...
    if(type == 3){
      imagePicker!.openNoCropCamera(context);
      return;
    }
    else{
      imagePicker!.openNoCropGallery(context);
      return;
    }
      ...
      child: Row(
        children: [
          ...
          Text(
            ...
            style: TextStyle(
                color: len > 0 ? Colors.black : MyColors.grey_color,
                fontFamily: 'Doomsday',
                fontSize: 14,
                fontWeight: FontWeight.bold),
          )
  ...
  submitIdCard() async {
    ...
    CommonUtils.generateIDVerificationCode();
    try {
  ...
  Widget verifyResultView() {
    ...
    return Container(
      ...
      child: Column(
        children: [
          _pending ? Icon(Icons.check_circle_outline, size: 70, color: MyColors.base_green_color,) : SizedBox(),
          Text(
            _submit
                ? "Thank you for providing the required information."
                : _pending
                ? "We are reviewing your submissions."
                : _accept
          ...
          Text(
            _submit
                ...
                : _pending
                ? "We'll get back to you or lift your limitation within 2 business days."
                : _accept
                ? "identity Successfully Verified."
                : "",
            textAlign: TextAlign.center,
  ...
  _body(BuildContext context) {
    bool isVisibleVerify =
        verifyStatus == Globals.DEFAULT || verifyStatus == Globals.REJECTED;
                      ...
                      : Column(
                          ...
                          children: [
                            ...
                            Text.rich(
                              TextSpan(
                                text:
                                    "Upload a valid government issued ID, driver's license or passport which includes your ",
                                style: _DESC_STYLE,
                                children: [
                                  TextSpan(
                            ...
                            SizedBox(height: _SPACING),
                            Text(
                              "Upload a document as proof of residential address such as your recent bank statement (less than 3 months old) which includes your name and your current address.",
                              style: _DESC_STYLE,
                            ),
                            SizedBox(height: _SPACING),
                            Text(
  ...
  userImage(File _image) {
    ...
    if(imagePickerMethod == 1){
      setState(() {
        gover_files = [_image];
      });
    }
    if(imagePickerMethod == 3){
      setState(() {
        verify_files = [_image];
      });
    }
    if(imagePickerMethod == 2){
      setState(() {
        proof_files = [_image];
      });
    }
