...
class ChatFile extends StatefulWidget {
  ...
  const ChatFile(
      {Key? key, this.receiverid = '', this.receiver = '', this.avatar = '', this.avatarTxt = ''})
      : super(key: key);
...
class ChatFileState extends State<ChatFile> {
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              FirebaseUtils.deleteMessage(
                  isCustomer != null && isCustomer! ? userid : selUsrId,
                  isCustomer != null && isCustomer! ? selUsrId : userid);
              Navigator.of(context).pop(false);
  ...
  void reSendDraftMessage() async {
    ...
    var draftMessages =
        messageBubbles.where((element) => element.isSending).toList();
      ...
      if (result.status == "true") {
        ...
        draftMessages.forEach((element) {
          ...
          String formattedDate =
              DateFormat('yyyy-MM-dd kk:mm:ss').format(tmpTime);
          var sentMessage = {
            'text': element.messageText ,
            'sender': element.messageSender,
            'read': false,
            'sent': isUserOnline
          };
  ...
  void getMessages() {
    print("GETMESSAGES");
    var _messageRef = FirebaseUtils.getMessageRef(
        isCustomer ? userid : widget.receiverid,
        isCustomer ? widget.receiverid : userid);
    _messageRef.onChildAdded.listen((event) {
      var snapshot = event.snapshot;
      if (snapshot.exists) {
        Map message = snapshot.value as Map<dynamic, dynamic>;
        try {
  ...
  void checkOnline() {
    if (checkOnlineTimer != null && checkOnlineTimer!.isActive)
      checkOnlineTimer!.cancel();
    checkOnlineTimer = Timer.periodic(
        Duration(seconds: CommonUtils.CHECK_ONLINE_SECONDS), (timer) {
      print("running timer => " + Globals.isInForeground.toString());
      if (!Globals.isInForeground) return;
      checkOnlineStatus();
      readMessage();
                  ...
                  child: SizedBox(
                      ...
                      child: Stack(
                        ...
                        children: [
                          CircleAvatar(
                            radius: 100,
                            backgroundImage: this._loadImageError
                                ? null
                          ...
                          Positioned(
                              ...
                              child: badges.Badge(
                                ...
                                badgeStyle: badges.BadgeStyle(
                                  ...
                                  badgeColor: isUserOnline
                                      ? MyColors.online_bg_color
                                      : MyColors.grey_color,
  ...
  void checkOnlineStatus() {
    try {
      ...
      FirebaseUtils.checkOnline(widget.receiverid).then((value) {
        ...
        bool typing = false;
        if (value.exists) {
          Map data = value.value as Map<dynamic, dynamic>;
          int status = data.containsKey("status")
              ? data['status']
              : CommonUtils.OFFLINE_STATUS;
          String date = data.containsKey("date") ? data['date'] : null;
          if (status == CommonUtils.OFFLINE_STATUS || date == null) {
            return;
          }
          var now = DateTime.now();
          var later = DateTime.parse(date).add(
              const Duration(seconds: CommonUtils.CHECK_ONLINE_SECONDS + 10));
  ...
  void readMessage() async {
    if (!CommonUtils.checkStringId(userid) ||
        !CommonUtils.checkStringId(widget.receiverid ?? '')) {
      return;
    }
                          ...
                          children: [
                            ...
                            Container(
                              child: Text(
                                messageTime == null
                                    ? "Just now"
                                    : CommonUtils.timesAgoFeature(messageTime!),
                                style: TextStyle(
                                  color: (isMe ?? false)
                                      ? Colors.white
                                      : Colors.grey,
