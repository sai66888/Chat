...
class ContactUsFileState extends State<ContactUsFile> {
      ...
      try {
        ...
        if (result.status == "true") {
          ...
          for (int i = 0; i < users.length; i++) {
            var snapshots = await FirebaseUtils.getMessage(
                users[i].user_id.toString(), userid ?? '');
  ...
  refresh() async {
    ...
    await (isCustomer ?? false ? getSupporters() : getCustomers());
    setState(() {
      isLoaded = true;
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: new AppBar(
        ...
        centerTitle: true,
        title: new Text(
          isCustomer ?? false ? "Support" : 'Customers',
          style: TextStyle(
            fontFamily: 'Doomsday',
            color: Colors.white,
            fontSize: 20,
  ...
  _body(BuildContext context) {
    var listData = List.from(isCustomer ?? false ? supporters : customers);
    return isLoaded
        ? ListView.builder(
