...
class GetFlutterwaveBankDetailsApi{
  NetworkUtils _netUtil = new NetworkUtils();

  Future<FlutterwaveBankDetailModel> search(String bankNumber, String accountNumber) {
    String baseTokenUrl = NetworkUtils.api_url + "getflutterwavebankaccountdetails?bankCode=${bankNumber}&accountNumber=${accountNumber}" ;
    return _netUtil
        .get(
      baseTokenUrl,
    )
      ...
      FlutterwaveBankDetailModel result = new FlutterwaveBankDetailModel.map(res);
      return result;
    });
  }
...
class FlutterwaveBankDetailModel {
  String status = '';
  String message = '';
  FlutterwaveBankDetail? bankdetail;

  FlutterwaveBankDetailModel.map(dynamic obj) {
    this.status = obj["status"].toString();
    this.message = obj["message"].toString();
    this.bankdetail =
    obj['data'] == null ? null : FlutterwaveBankDetail.fromJson(obj['data']);
  }
}

class FlutterwaveBankDetail {
  String? account_number;
  String? account_name;



  FlutterwaveBankDetail.fromJson(dynamic jsonMap)
      :account_number = jsonMap['account_number'].toString(),
...
class GetFlutterWaveBankListApi{
  dynamic bankList = {
    "status": "success",
  ...
  NetworkUtils _netUtil = new NetworkUtils();
  FlutterwaveBankListModel search() {
    FlutterwaveBankListModel result = new FlutterwaveBankListModel.map(bankList);
    return result;
  }
}

class FlutterwaveBankListModel {
  String status = '';
  String message = '';
  List<FlutterwaveBankData> banklist = [];

  FlutterwaveBankListModel.map(dynamic obj) {
    this.status = obj["status"].toString();
    this.message = obj["message"].toString();
    this.banklist =
        (obj['data'] as List).map((i) => FlutterwaveBankData.fromJson(i)).toList();
  }
}

class FlutterwaveBankData {
  final int id;
  final String name;
  final String code;
  ...
  FlutterwaveBankData.fromJson(Map jsonMap)
